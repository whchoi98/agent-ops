...
export function ProjectDialog({ project, onClose }: { project?: Project; onClose: () => void }) {
  ...
  return <Dialog title={project ? t("프로젝트 편집") : t("프로젝트 추가")} description={t("대화를 정리하고 에이전트를 실행할 작업 디렉터리를 등록하세요.")}
    onClose={onClose} size="medium" footer={<><Button onClick={onClose} disabled={busy}><Trans message={"취소"} /></Button>
      <Button variant="primary" type="submit" form="project-form" icon={project ? Save : FolderPlus} busy={busy}>{project ? t("프로젝트 저장") : t("프로젝트 추가")}</Button></>}>
    <form id="project-form" className="form-stack" onSubmit={save}>
      <Field label={t("프로젝트 이름")} htmlFor="project-name"><input id="project-name" autoFocus data-autofocus required maxLength={200}
        value={name} onChange={event => setName(event.target.value)} placeholder={t("프로젝트 이름")} /></Field>
      <Field label={t("프로젝트 경로")} htmlFor="project-path" hint={project ? t("등록된 작업 디렉터리입니다.") : t("이 컴퓨터에 존재하는 디렉터리의 전체 경로를 입력하세요.")}>
      ...
      <fieldset className="field"><legend><Trans message={"프로젝트 색상"} /></legend><div className="color-picker">
        {COLORS.map(value => <button type="button" key={value} aria-label={t("{0} 색상", { "0": value })} aria-pressed={color === value}
        ...
        <label className="custom-color"><span><Trans message={"직접 선택"} /></span><input type="color" aria-label={t("프로젝트 색상 직접 선택")} value={color} onChange={event => setColor(event.target.value)} /></label>
      </div></fieldset>
      <label className={`execution-permission ${enabled ? 'permission-checked' : ''}`}><input type="checkbox" checked={enabled} onChange={event => setEnabled(event.target.checked)} />
        <span><strong><Trans message={"이 프로젝트에서 에이전트 실행 허용"} /></strong><small><Trans message={"허용하면 선택한 디렉터리에서 CLI 작업을 시작할 수 있습니다. 실행할 때마다 명령을 먼저 확인합니다."} /></small></span></label>
