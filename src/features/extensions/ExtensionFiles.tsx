...
function FilePreview({ extensionId, file, projectId }: { extensionId: string; file: ExtensionFile; projectId?: string }) {
  ...
  return <section className="extension-file-preview" aria-label={`${file.path} 미리보기`} aria-busy={resource.loading}>
...
export function ExtensionFiles({ extensionId, files, projectId }: {
  ...
  if (!files.length) return <EmptyState compact icon={FileText} title="표시할 파일이 없습니다"
    description="이 항목에서 발견한 파일이 없습니다. 내용 분석에서 검색 진단을 확인하세요." />;
  return <>
    <p className="extension-file-hint">파일을 선택하면 읽기 전용으로 내용을 확인합니다. 미리보기가 제한된 파일은 열 수 없습니다.</p>
    <div className="extension-files">
      <div className="extension-file-list" role="group" aria-label="미리보기 파일 선택">
        {files.map(file => <button type="button" key={file.id}
          ...
          aria-label={`${file.path} 파일 보기`} title={file.readable ? file.path : '이 파일은 텍스트 미리보기가 제한됩니다.'}
          onClick={() => setSelectedId(file.id)}>
          <FileText size={16} aria-hidden /><span><code>{file.path}</code>
            <small>{FILE_KIND_LABELS[file.kind]} · {formatBytes(file.bytes)}{!file.readable && ' · 미리보기 제한'}</small>
          </span>
        </button>)}
      </div>
      {selected ? <FilePreview key={JSON.stringify([extensionId, selected.id, projectId])}
        ...
        : <EmptyState compact icon={FileText} title="파일을 선택하세요"
          description="지시문, 참고 자료와 설정 파일의 내용을 확인할 수 있습니다." />}
    </div>
  </>;
}
