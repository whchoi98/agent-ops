...
import { cleanTerminal, isActiveRun, number } from '../../lib/format';
...
export function RunDetailDialog({ id }: { id: string }) {
  ...
  const events = useMemo(() => (resource.data?.events ?? []).filter(event => {
    const raw = cleanTerminal(event.text);
    const searchable = event.stream === 'system' ? `${raw}\n${notice(raw)}` : raw;
    return (stream === 'all' || event.stream === stream) && (!search || searchable.toLocaleLowerCase().includes(search.toLocaleLowerCase()));
  }), [resource.data?.events, stream, search, notice]);
  ...
  return <Dialog title={run?.title || t("실행 불러오는 중")} size="drawer" onClose={closeModal} className="run-detail-dialog"
    ...
    footer={run && <><span className="run-footer-status">{isActiveRun(run.status) ? t("이 화면을 닫아도 작업은 계속됩니다.") : run.exitCode !== null ? t("프로세스 종료 코드 {0}", { "0": run.exitCode }) : t("실행 기록이 저장되었습니다.")}</span>
      <Button onClick={closeModal}><Trans message={"닫기"} /></Button><RunActions run={run} onChanged={resource.reload} /></>}>
    ...
    {!run ? <Skeleton rows={8} /> : <>
      ...
      <section className="run-log-section" aria-label={t("실행 로그")}>
        <div className="run-log-title"><div><Terminal size={17} aria-hidden /><h3><Trans message={"출력 로그"} /></h3>
          <span><Trans message={"{0}개 이벤트"} values={{ "0": number(resource.data?.events.length ?? 0) }} /></span>{run.status === 'running' && <span className="log-live"><i className="live-dot" />LIVE</span>}</div>
          <CopyButton compact text={events.map(event => `[${event.timestamp}] [${event.stream}] ${cleanTerminal(event.text)}`).join('\n')} label={t("표시된 로그 복사")} /></div>
        <div className="log-toolbar"><select aria-label={t("로그 스트림 필터")} value={stream} onChange={event => setStream(event.target.value as typeof stream)}>
          <option value="all"><Trans message={"모든 출력"} /></option><option value="stdout">stdout</option><option value="stderr">stderr</option><option value="system">system</option></select>
          <div className="search-input log-search"><Search size={14} aria-hidden /><input value={search} aria-label={t("실행 로그 검색")} placeholder={t("로그 검색")}
            onChange={event => setSearch(event.target.value)} />{search && <IconButton icon={X} label={t("로그 검색 지우기")} onClick={() => setSearch('')} />}</div>
          <label className="follow-control"><input type="checkbox" checked={follow} onChange={event => setFollow(event.target.checked)} /><Trans message={"자동 스크롤"} /></label>
        </div>
        <div className="log-output" ref={scrollRef} tabIndex={0} role="region" aria-label={t("실행 출력")}
          ...
          {events.length ? events.map(event => <div key={event.id} className={`log-line log-${event.stream}`}>
            ...
            <span className="log-stream">{event.stream}</span><pre>{highlightText(event.stream === 'system' ? notice(cleanTerminal(event.text)) : cleanTerminal(event.text), search)}</pre>
          </div>) : <div className="log-empty"><Terminal size={28} aria-hidden /><strong>{search || stream !== 'all' ? t("조건에 맞는 로그가 없습니다") : run.status === 'queued' ? t("작업이 대기 중입니다") : t("아직 출력 로그가 없습니다")}</strong>
            <p>{search || stream !== 'all' ? t("검색어와 출력 필터를 확인하세요.") : run.status === 'queued' ? t("동시 실행 한도 또는 같은 프로젝트의 작업이 끝나면 시작합니다.") : isActiveRun(run.status) ? t("CLI가 출력하면 자동으로 표시됩니다.") : t("이 실행에 남아 있는 출력이 없습니다.")}</p></div>}
        </div>
      </section>
    </>}
  </Dialog>;
