...
export function NewRunDialog({ draft }: { draft: NewRunDraft }) {
  ...
  return <Dialog title={draft.resumeSessionId ? t("세션 이어가기") : draft.sourceSessionId ? t("전달받은 맥락으로 새 실행") : t("새 실행")}
    description={t("작업을 준비하고, 실행할 명령을 확인하세요.")} size="large" onClose={closeModal} className="new-run-dialog"
    footer={<div className="new-run-footer">
      ...
      <div><Button onClick={closeModal} disabled={!!busy}><Trans message={"닫기"} /></Button>
        <Button type="submit" form="new-run-form" icon={Eye} busy={busy === 'preview'} disabled={busy === 'execute' || !validForm}><Trans message={"명령 미리보기"} /></Button>
    ...
    <form id="new-run-form" onSubmit={prepare} className="form-stack">
      ...
      {draft.resumeSessionId && <InlineNotice><Trans message={"선택한 CLI 세션의 맥락을 이어갑니다. 에이전트와 프로젝트는 원래 세션을 따릅니다."} /></InlineNotice>}
      {draft.sourceSessionId && <InlineNotice><Trans message={"전달받은 프롬프트를 검토하고 다음 작업에 맞게 수정하세요. 아직 실행되지 않았습니다."} /></InlineNotice>}
      <fieldset disabled={!!busy} className="form-stack form-reset">
        ...
        <div className="form-grid">
          <Field label={t("프로젝트")} htmlFor="run-project"><select id="run-project" required value={projectId} disabled={!!draft.resumeSessionId && !!draft.projectId}
          ...
          <Field label={t("템플릿 (선택)")} htmlFor="run-template"><select id="run-template" value={templateId} onChange={event => applyTemplate(event.target.value)}>
            <option value=""><Trans message={"직접 작성"} /></option>{data.templates.map(template => <option key={template.id} value={template.id}>{template.name}</option>)}
          </select></Field>
        </div>
        ...
        <div className="form-grid">
          ...
          <Field label={t("모델 (선택)")} htmlFor="run-model" hint={t("비워두면 CLI의 기본 모델을 사용합니다.")}><input id="run-model" value={model} list="run-models"
            maxLength={160} placeholder={t("CLI 기본 모델")} onChange={event => setModel(event.target.value)} /><datalist id="run-models">{models.map(item => <option value={item} key={item} />)}</datalist></Field>
        </div>
        ...
        <Field label={t("프롬프트")} htmlFor="run-prompt"><textarea id="run-prompt" className="prompt-textarea" required rows={7} maxLength={64_000}
          ...
          placeholder={draft.resumeSessionId ? t("이어서 진행할 작업과 원하는 결과를 적어주세요.") : t("에이전트가 수행할 작업, 범위, 원하는 결과를 구체적으로 적어주세요.")} />
          <div className="prompt-meta"><span>{draft.sourceSessionId ? t("전달된 맥락을 자유롭게 수정할 수 있습니다.") : t("구체적인 목표와 범위가 좋은 실행의 시작입니다.")}</span><span className="numeric">{number(prompt.length)} / 64,000</span></div>
        </Field>
      </fieldset>
