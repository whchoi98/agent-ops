...
import { dateTime, duration, money, number, recordedDuration } from '../../lib/format';
...
export function CompareDialog({ ids }: { ids: [string, string] }) {
  ...
  const metric = (value: number | null) => value === null ? <span className="text-muted">미기록</span> : number(value);
  const rows: Array<{ label: string; render: (session: SessionDetail) => ReactNode }> = [
    ...
    { label: '프로젝트', render: session => session.projectName || '미지정' },
  ...
  return <Dialog title="세션 비교" description="두 세션에 기록된 모델, 사용량과 시간을 나란히 확인하세요." onClose={closeModal} size="large" className="compare-dialog">
    {resource.error ? <ErrorState message={resource.error} retry={resource.reload} /> : !resource.data ? <Skeleton rows={8} /> : <>
      <InlineNotice>미기록은 0이 아닙니다. 기록 시간 범위에는 대화 사이의 대기 시간도 포함됩니다.</InlineNotice>
      <div className="comparison-scroll"><table className="comparison-table"><thead><tr><th>비교 항목</th>{resource.data.map(session =>
        <th key={session.id}><span>{session.title}</span><Button variant="ghost" size="small" icon={ArrowUpRight} onClick={() => openSession(session.id)}>대화 열기</Button></th>)}</tr></thead>
        <tbody>{rows.map(row => <tr key={row.label}><th>{row.label}</th>{resource.data!.map(session => <td key={session.id}>{row.render(session)}</td>)}</tr>)}</tbody>
      </table></div>
    </>}
  </Dialog>;
}
