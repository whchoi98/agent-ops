...
export function HandoffDialog({ session, onClose }: { session: SessionDetail; onClose: () => void }) {
  ...
  return <Dialog title="다른 에이전트에 전달" description="대화의 맥락을 새 프롬프트로 준비합니다." size="medium" onClose={onClose}
    footer={<><Button onClick={onClose} disabled={busy}>닫기</Button><Button type="submit" form="handoff-form" icon={ArrowRightLeft} variant="primary" busy={busy}>전달 프롬프트 준비</Button></>}>
    <form id="handoff-form" onSubmit={prepare} className="form-stack">
      ...
      <fieldset className="field"><legend>전달받을 에이전트</legend><div className="agent-picker">
      ...
      <Field label="추가 지시사항 (선택)" htmlFor="handoff-instruction"><textarea id="handoff-instruction" rows={4} maxLength={8000} value={instruction}
        onChange={event => setInstruction(event.target.value)} placeholder="전달받은 에이전트가 집중할 내용이나 다음 작업을 적어주세요." /></Field>
      <InlineNotice>준비된 프롬프트를 다음 화면에서 수정할 수 있습니다. 명령을 미리 보고 실행을 눌러야 작업이 시작됩니다.</InlineNotice>
