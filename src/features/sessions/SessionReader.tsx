...
import { AGENT_META, dateTime, number } from '../../lib/format';
...
export function SessionReader({ session }: { session: SessionDetail }) {
  ...
  return <div className="session-reader">
    <div className="reader-toolbar">
      <div className="role-filters" aria-label="메시지 역할 필터" title="전체 세션의 역할별 메시지 수">{ROLES.map(item => {
        ...
        return <button type="button" key={item.value} aria-pressed={role === item.value} className={role === item.value ? 'active' : ''}
          onClick={() => changeRole(item.value)}>{item.label}<span>{count === null ? '—' : number(count)}</span></button>;
      })}</div>
      <div className="reader-find">
        <Search size={15} aria-hidden /><input aria-label="대화에서 찾기" placeholder="전체 대화에서 찾기" maxLength={500}
        ...
        {find && <><span className="find-count" aria-live="polite">{loading ? '검색 중…' : total
          ? `${number(Math.min(activeMatch + 1, total))} / ${number(total)}` : '0개'}</span>
          <IconButton label="이전 검색 결과" icon={ChevronUp} disabled={!canNavigate || !total} onClick={() => move(-1)} />
          <IconButton label="다음 검색 결과" icon={ChevronDown} disabled={!canNavigate || !total} onClick={() => move(1)} />
          <IconButton label="대화 찾기 초기화" icon={X} onClick={() => changeFind('')} /></>}
      </div>
    </div>
    <div className={`reader-result-summary ${searching || debouncing ? 'reader-search-active' : ''}`}>
      {searching || debouncing
        ? '전체 기록 검색 · 일치하는 메시지만 표시합니다.'
        : '기록순 50개씩 · 역할별 개수는 전체 세션 기준'}
    </div>
        ...
        : loading ? <Skeleton rows={5} />
          : messages.length && page ? <div className="conversation-messages" key={requestKey}>
            ...
            {messages.map((message, index) => <article key={message.id}
              ...
              aria-label={`${message.role === 'user' ? '사용자' : message.role === 'assistant' ? AGENT_META[session.agent].name : message.role === 'tool' ? '도구' : '시스템'} 메시지`}>
              ...
              <div className="message-main"><div className="message-meta">
                <strong>{message.role === 'user' ? '사용자' : message.role === 'assistant' ? AGENT_META[session.agent].name : message.role === 'tool' ? message.toolName || '도구' : '시스템'}</strong>
            ...
            <div className="conversation-end"><span />{hasNext ? '다음 페이지에서 계속됩니다' : searching
              ? '검색 결과의 끝' : role === 'all' ? '기록의 끝' : '선택한 역할의 기록 끝'}<span /></div>
          </div> : <EmptyState compact title={searching ? '검색어가 포함된 메시지가 없습니다' : '표시할 메시지가 없습니다'}
            description={searching ? '전체 세션을 검색했습니다. 다른 검색어나 역할을 선택해 보세요.'
              : role === 'all' ? '이 세션에는 대화 내용이 기록되지 않았습니다.' : '다른 역할을 선택해 대화를 확인하세요.'}
            action={searching ? <Button size="small" onClick={() => changeFind('')}>검색 초기화</Button>
              : role !== 'all' ? <Button size="small" onClick={() => changeRole('all')}>모든 역할 보기</Button> : undefined} />}
    </div>
    <nav className="reader-pagination" aria-label="메시지 페이지">
      <span className="reader-page-range" role="status" aria-live="polite">{loading ? '메시지 불러오는 중…' : resource.error
        ? '메시지를 다시 불러와 주세요.' : page && messages.length
          ? `${number(page.offset + 1)}–${number(page.offset + messages.length)} / ${number(page.total)}개 ${searching ? '검색 결과' : '메시지'}`
          : `0개 ${searching ? '검색 결과' : '메시지'}`}</span>
      <div className="reader-page-actions">
        <IconButton label="처음 메시지" icon={ChevronsLeft} disabled={!canNavigate || !page || page.offset === 0}
          onClick={() => goToPage(0)} />
        <Button size="small" icon={ChevronLeft} disabled={!canNavigate || !page || page.offset === 0}
          onClick={() => goToPage((page?.offset ?? 0) - PAGE_SIZE)}>이전 메시지</Button>
        <Button size="small" disabled={!canNavigate || !hasNext}
          onClick={() => goToPage((page?.offset ?? 0) + PAGE_SIZE)}>다음 메시지<ChevronRight size={14} aria-hidden /></Button>
        <IconButton label="마지막 메시지" icon={ChevronsRight} disabled={!canNavigate || !hasNext}
