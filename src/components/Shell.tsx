...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return <>
    <Brand />
    <div className="workspace-label"><span className="workspace-avatar">W</span><div>내 워크스페이스<small>Personal workspace</small></div>
      <ShieldCheck size={15} aria-hidden />
    </div>
    <nav aria-label="주요 메뉴" className="main-nav">
      <span className="nav-caption">워크스페이스</span>
      {NAV_ITEMS.map(({ page: item, label, icon: Icon }, index) => <div key={item}>
        {index === 4 && <span className="nav-caption nav-caption-secondary">분석 및 관리</span>}
        <a href={`#/${item}`} onClick={onNavigate} className={`nav-item ${page === item ? 'nav-active' : ''}`}
          aria-current={page === item ? 'page' : undefined}>
          <Icon size={18} strokeWidth={page === item ? 2.1 : 1.7} aria-hidden /><span>{label}</span>
    ...
    <div className="sidebar-bottom">
      <button className="sidebar-shortcut" onClick={() => { onNavigate?.(); setPaletteOpen(true); }}>
        <Command size={17} aria-hidden /><span>빠른 이동</span><kbd>{SEARCH_SHORTCUT}</kbd>
      </button>
      <div className="local-workspace-card">
        <div><ShieldCheck size={16} aria-hidden /><strong>내 컴퓨터에 보관</strong></div>
        <p>대화와 작업 기록을 한곳에서.</p>
        <span className="sidebar-connection"><i className={connection === 'connected' ? 'connection-on' : ''} />
          {connection === 'connected' ? '실시간 갱신 중' : connection === 'connecting' ? '서버 확인 중' : '갱신 재연결 중'}</span>
      </div>
...
export function Shell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    setMobileMenu(false);
    document.title = `${PAGE_NAMES[page]} · Agent Ops`;
  ...
  return <div className="app-shell">
    ...
    {mobileMenu && <Dialog title="워크스페이스 메뉴" onClose={() => setMobileMenu(false)} className="mobile-sidebar-dialog" size="small" bodyClassName="mobile-sidebar-content">
      <SidebarContent onNavigate={() => setMobileMenu(false)} />
    </Dialog>}
    <div className="workspace">
      <header className="topbar">
        <IconButton className="mobile-menu-button" icon={Menu} label="메뉴 열기" onClick={() => setMobileMenu(true)} />
        <div className="breadcrumb"><span>워크스페이스</span><ChevronRight size={13} aria-hidden /><strong>{PAGE_NAMES[page]}</strong></div>
        <div className="topbar-actions">
          <button className="global-search-button" aria-label={`빠른 검색 및 이동 (${SEARCH_SHORTCUT})`} onClick={() => setPaletteOpen(true)}>
            <Search size={16} aria-hidden /><span>빠른 검색 및 이동</span><kbd>{SEARCH_SHORTCUT}</kbd>
          </button>
          <IconButton className="topbar-theme-button" label={dark ? '라이트 모드로 전환' : '다크 모드로 전환'}
            icon={dark ? Sun : Moon} busy={themeSaving} disabled={!data} onClick={() => void setTheme(dark ? 'light' : 'dark')} />
          <IconButton className="topbar-sync-button" label="세션 동기화" icon={RefreshCw} busy={syncing} disabled={!data} onClick={() => void sync()} />
          <span className="topbar-divider" />
          <Button variant="primary" icon={Plus} onClick={() => openNewRun()} disabled={!data}>새 실행</Button>
        </div>
      </header>
      <main id="main-content" tabIndex={-1} className="main-content">
        {data?.demo && <div className="demo-banner" role="status">
          <span className="demo-icon"><FlaskConical size={20} aria-hidden /></span>
          <div><strong><span className="demo-label">DEMO</span>데모 워크스페이스</strong>
            <p>샘플 데이터로 둘러보세요. 정리와 명령 미리보기는 가능하며 실제 에이전트는 실행되지 않습니다.</p>
          </div>
          <button className="demo-action" onClick={() => openNewRun()}>실행 흐름 살펴보기<ArrowUpRight size={15} aria-hidden /></button>
        </div>}
        {data && error && <InlineNotice tone="warning"><div className="refresh-error">
          <span>최신 상태를 가져오지 못했습니다. 마지막으로 불러온 기록을 표시합니다.</span>
          <Button size="small" icon={RefreshCw} busy={refreshing} onClick={() => void refresh()}>다시 연결</Button>
        </div></InlineNotice>}
        <div className="page-content" key={page}>{children}</div>
        {data && <footer className="workspace-footer"><span>Agent Ops <span>·</span> 로컬 워크스페이스</span><span>Codex / Claude Code / Kiro CLI</span></footer>}
      </main>
    </div>
  </div>;
}
