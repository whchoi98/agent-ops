...
export function CommandPalette() {
  ...
  if (data && (!lowered || '새 실행 new run 작업 시작'.includes(lowered))) items.push({
    id: 'new-run', label: '새 실행', description: '에이전트에게 새 작업 맡기기', icon: <Plus size={18} />, group: '빠른 실행',
    run: () => choose(() => openNewRun()),
  });
  for (const item of NAV_ITEMS) {
    if (!lowered || `${item.label} ${item.english}`.toLocaleLowerCase().includes(lowered)) items.push({
      id: item.page, label: item.label, description: item.english, icon: <item.icon size={18} />, group: '페이지',
      run: () => choose(() => navigate(item.page)),
    });
  }
  if (lowered) {
    items.push({ id: 'search-all', label: `“${query.trim()}” 전체 검색`, description: '제목과 전체 대화 내용에서 찾기', icon: <Search size={18} />,
      group: '세션 검색', run: () => choose(() => navigate('sessions', { q: query.trim() })) });
    if (debounced === query.trim() && resource.data) for (const session of resource.data.items) items.push({
      ...
      icon: <ProviderMark agent={session.agent} />, group: '세션 검색', run: () => choose(() => openSession(session.id)),
    });
  }
  ...
  return <Dialog title="빠른 검색 및 이동" onClose={close} size="medium" className="command-palette" hideHeading
    footer={<div className="palette-help"><span><kbd>↑</kbd><kbd>↓</kbd> 이동</span><span><kbd>↵</kbd> 선택</span><span><kbd>esc</kbd> 닫기</span></div>}>
    <div className="palette-input"><Search size={20} aria-hidden /><input autoFocus data-autofocus value={query} maxLength={500} placeholder="페이지, 세션 검색 또는 새 실행…"
      aria-label="빠른 검색 및 이동" role="combobox" aria-expanded aria-controls={`${id}-results`} aria-autocomplete="list"
    ...
    <div className="palette-results" role="listbox" id={`${id}-results`} aria-label="빠른 이동 결과">
      ...
      {lowered && (resource.loading || debounced !== query.trim()) && <div className="palette-feedback"><LoaderCircle size={14} className="spin" aria-hidden />세션 검색 중…</div>}
      {lowered && resource.error && <div className="palette-feedback palette-error" role="status">세션 검색에 연결하지 못했습니다. <button onClick={resource.reload}>다시 시도</button></div>}
      {lowered && !resource.loading && resource.data?.items.length === 0 && <div className="palette-feedback">일치하는 세션이 없습니다. 검색어를 바꿔보세요.</div>}
    </div>
  </Dialog>;
}
