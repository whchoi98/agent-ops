...
export function Sessions() {
  ...
  return <>
    <PageHeading title="세션 탐색" description="대화의 전체 내용을 검색하고, 필요한 맥락에서 다시 시작하세요." eyebrow="SESSION EXPLORER"
      actions={<Button icon={RefreshCw} busy={syncing} onClick={() => void sync()}>세션 동기화</Button>} />
    <section className="panel session-explorer">
      <div className="explorer-search-row"><div className="search-input search-input-large">
        <Search size={19} aria-hidden /><input aria-label="세션 전체 내용 검색" maxLength={500} placeholder="제목과 대화 내용 검색…" value={query.q ?? ''}
          onChange={event => update({ q: event.target.value })} />
        {query.q && <IconButton label="검색어 지우기" icon={X} onClick={() => update({ q: undefined })} />}
        <span className="search-scope">전체 대화 검색</span>
      </div></div>
      <div className="explorer-toolbar">
        <div className="provider-tabs" aria-label="에이전트 필터">
          <button aria-label="전체 에이전트" aria-pressed={!query.agent} className={!query.agent ? 'active' : ''} onClick={() => update({ agent: undefined })}>
            <span className="provider-all-desktop">전체 에이전트</span><span className="provider-all-mobile">전체</span>
          </button>
        ...
        <div className="explorer-quick-filters">
          <select aria-label="프로젝트 필터" value={query.project ?? ''} onChange={event => update({ project: event.target.value || undefined })}>
            <option value="">모든 프로젝트</option>{data.projects.map(project => <option key={project.id} value={project.path}>{project.name}</option>)}
          </select>
          <button className={`filter-button ${query.bookmarked ? 'filter-active' : ''}`} aria-pressed={!!query.bookmarked}
            onClick={() => update({ bookmarked: query.bookmarked ? undefined : true })}><Bookmark size={15} aria-hidden />북마크</button>
          <button className={`filter-button ${expanded || advancedCount ? 'filter-active' : ''}`} aria-expanded={expanded} aria-controls="advanced-session-filters"
            onClick={() => setExpanded(value => !value)}><SlidersHorizontal size={15} aria-hidden />필터{advancedCount > 0 && <span>{advancedCount}</span>}</button>
        </div>
      </div>
      {expanded && <div className="advanced-filters" id="advanced-session-filters">
        <Field label="상태" htmlFor="session-status"><select id="session-status" value={query.status ?? ''}
          onChange={event => update({ status: (event.target.value || undefined) as SessionQuery['status'] })}>
          <option value="">모든 상태</option><option value="recorded">기록됨</option><option value="completed">완료</option><option value="failed">실패</option>
        </select></Field>
        <Field label="태그" htmlFor="session-tag"><input id="session-tag" maxLength={60} value={query.tag ?? ''} list="known-session-tags"
          placeholder="태그 이름" onChange={event => update({ tag: event.target.value || undefined })} />
          <datalist id="known-session-tags">{knownTags.map(tag => <option value={tag} key={tag} />)}</datalist>
        </Field>
        <Field label="시작일" htmlFor="session-since"><input id="session-since" type="date" value={inputDate(query.since)}
          max={inputDate(query.until) || undefined} onChange={event => update({ since: localDateBoundary(event.target.value) })} /></Field>
        <Field label="종료일" htmlFor="session-until"><input id="session-until" type="date" value={inputDate(query.until)}
          min={inputDate(query.since) || undefined} onChange={event => update({ until: localDateBoundary(event.target.value, true) })} /></Field>
        <div className="filter-reset"><button className="text-button" onClick={clear}>필터 초기화</button><span>날짜는 현지 시간 기준</span></div>
      </div>}
      <div className="results-heading"><span><strong>{number(total)}</strong>개 세션{isSearching && <span className="results-pending">검색 중…</span>}</span>
        <div>{filtered && <button className="text-button clear-filters" onClick={clear}><X size={12} aria-hidden />조건 지우기</button>}
          <label className="sort-control"><ListFilter size={14} aria-hidden /><span className="sr-only">세션 정렬</span><select value={query.sort}
            onChange={event => update({ sort: event.target.value as SessionQuery['sort'] })}>
            <option value="recent">최근 기록순</option><option value="oldest">오래된 기록순</option><option value="tokens">토큰 많은 순</option>
          </select></label>
        </div>
      </div>
      {resource.error ? <ErrorState message={resource.error} retry={resource.reload} /> : !resource.data ? <Skeleton rows={7} className="table-skeleton" />
        ...
        </div> : <EmptyState icon={Search} title={filtered ? '검색 조건에 맞는 세션이 없습니다' : '아직 세션이 없습니다'}
          description={filtered ? '검색어를 바꾸거나 필터를 초기화해 보세요.' : '기록 경로를 설정하고 동기화하면 세션이 표시됩니다.'}
          action={filtered ? <Button onClick={clear}>필터 초기화</Button> : <Button onClick={() => navigate('settings')}>기록 경로 설정</Button>} />}
      {resource.data && total > 0 && <div className="pagination">
        <div className="pagination-summary"><span>{number(Math.min(offset + 1, total))}–{number(Math.min(offset + limit, total))} / {number(total)}</span>
          <select aria-label="페이지당 세션 수" value={limit} onChange={event => update({ limit: Number(event.target.value) })}>
            <option value={20}>20개씩</option><option value={40}>40개씩</option><option value={60}>60개씩</option>
          </select>
        </div>
        <nav aria-label="세션 페이지" className="pagination-controls">
          <IconButton label="이전 페이지" icon={ChevronLeft} disabled={offset === 0 || isSearching} onClick={() => update({ offset: Math.max(0, offset - limit) })} />
          {pageNumbers.map((item, index) => <span key={item} className="page-number-item">
            ...
            <button className={`page-number ${page === item ? 'current' : ''}`} aria-label={`${item}페이지`}
              aria-current={page === item ? 'page' : undefined} disabled={isSearching} onClick={() => update({ offset: (item - 1) * limit })}>{item}</button>
          </span>)}
          <IconButton label="다음 페이지" icon={ChevronRight} disabled={offset + limit >= total || isSearching} onClick={() => update({ offset: offset + limit })} />
        </nav>
      </div>}
    </section>
    <p className="page-footnote">체크박스로 세션 2개를 선택하면 모델, 사용량과 기록 시간을 비교할 수 있습니다. <span>미기록은 사용량 0을 뜻하지 않습니다.</span></p>
    {selection.length > 0 && <div className="selection-tray">
      <span className="selection-icon"><Check size={17} aria-hidden /></span><strong>{selection.length}개 선택</strong>
      <div className="selection-items">{selection.map(session => <span key={session.id}><ProviderMark agent={session.agent} size="small" />
        <span>{session.title}</span><IconButton icon={X} label={`${session.title} 비교 선택 해제`} onClick={() => select(session)} /></span>)}</div>
      <Button variant="primary" size="small" icon={GitCompareArrows} disabled={selection.length !== 2}
        onClick={() => openCompare([selection[0].id, selection[1].id])}>세션 비교</Button>
      <IconButton icon={X} label="선택 모두 해제" onClick={() => setSelection([])} />
    </div>}
  </>;
}
