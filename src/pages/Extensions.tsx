...
export function Extensions() {
  ...
  return <div className="extensions-workspace">
    <PageHeading title="스킬·플러그인" eyebrow="ASSISTANT EXTENSIONS"
      description="어시스턴트의 확장을 찾아보고, 지시문과 설정 근거를 확인하세요."
      actions={<Button icon={RefreshCw} busy={refreshing} disabled={pending} onClick={() => void refreshCatalog()}>목록 새로고침</Button>} />
    <InlineNotice>{USAGE_NOTICE}
      {catalog?.usageNotice && catalog.usageNotice !== USAGE_NOTICE && <p>{catalog.usageNotice}</p>}
    </InlineNotice>
    ...
    <section className="panel extension-explorer" aria-label="스킬·플러그인 목록">
      <ExtensionFilters query={query} projects={projects} update={update} />
      <div className="extension-results-heading">
        <h2 aria-live="polite">{pending ? '목록 확인 중…' : catalog ? <><span className="numeric">{number(catalog.total)}</span>개 항목</> : '목록 확인 실패'}</h2>
        {filtered && <button type="button" className="text-button" onClick={clear}><X size={13} aria-hidden />필터 초기화</button>}
      </div>
            ...
            : catalog.items.length ? <ExtensionList items={catalog.items} pending={pending} onSelect={select} />
              : pending ? <Skeleton rows={4} />
                : <EmptyState icon={filtered ? Search : Puzzle}
                  title={filtered ? '조건에 맞는 확장이 없습니다' : '발견한 확장이 없습니다'}
                  description={filtered ? '검색어나 필터를 바꿔보세요.' : '등록된 프로젝트를 선택하거나 검색 경로와 진단을 확인한 뒤 목록을 새로고침하세요.'}
                  action={filtered ? <Button onClick={clear}>필터 초기화</Button>
                    : <Button icon={RefreshCw} busy={refreshing} onClick={() => void refreshCatalog()}>목록 새로고침</Button>} />}
      </div>
    ...
    {selected && <Suspense fallback={<div className="modal-loading" role="status">확장 내용 불러오는 중…</div>}>
